...
% Simulación 2
%
% Sobre el escenario 2, con curva, modifico la distancia de separación
% Obtengo
...
R1=50; % radio de curva [m]
R2=R1+AnchoCarril;
dmin=6;   % distancia mínima
dmax=50;  % distancia máxima
dd=0.1;   % separación entre posiciones
          
pesosEGC=[0.5 0.5]; % pesos EGC (Equal Gain Combining)

Hright=[];Hleft=[];Hegc=[];Hmrc=[];Hsb=[];  %Heigths
IN2=[];IN2egc=[];IN2mrc=[];IN2sb=[];        %Pesos
for d=dmin:dd:dmax
    [DTX,DRX]=GeneraEscenario2(d,R1,R2,ParamTxRx);
...
title(['CURVA DOBLE CARRIL R=', num2str(R1), 'm: (FOV=', num2str(DRX(1,7)*180/pi), '^o, HPSA=' num2str(HSPA),'^o, P_{piloto}=' num2str(DTX(1,8)), 'W, A=', num2str(DRX(1,8)),'m^2)'])
...
figure;
plot(xd,-20*log10(Hegc),'b');
hold on;grid;
plot(xd,-20*log10(Hsb),'r');
plot(xd,-20*log10(Hmrc),'k');
title(['CURVA DOBLE CARRIL R=', num2str(R1), 'm: (FOV=', num2str(DRX(1,7)*180/pi), '^o, HPSA=' num2str(HSPA),'^o, A=', num2str(DRX(1,8)),'m^2)'])
xlabel('Distancia entre vehículos [m]');
