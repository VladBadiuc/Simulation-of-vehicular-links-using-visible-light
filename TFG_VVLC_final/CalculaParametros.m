...
M=tamano_tx(1);
%fprintf("numero tx M --> %6.2f \r\n",M);
tamano_rx=size(DatosRX);
N=tamano_rx(1);
%fprintf("numero rx N --> %6.2f \r\n",N);

% inicializo matrices
H0=zeros(N,M); TAU=zeros(N,M); 
for k=1:N                    % para los N=2 receptores
     for p=1:M               % para los M transmisores
         rtx=DatosTX(p,1:3); %fprintf("%rtx --> %6.2f",rtx); % posición piloto
         ntx=DatosTX(p,4:6); %fprintf("%ntx --> %6.2f",ntx);% vector unitario piloto
         rrx=DatosRX(k,1:3); %fprintf("%rrx --> %6.2f",rrx);% posición detector
         nrx=DatosRX(k,4:6); %fprintf("%rtx --> %6.2f",rrx);% vector posición detector
         vrxtx=rrx-rtx;     % vector detector-piloto 
         dist=norm(vrxtx);  %fprintf("%rtx --> %6.2f",dist);% ditancia detector-piloto
